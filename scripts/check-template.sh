...
for file in AGENTS.md README.md .env.example package.json tsconfig.json wrangler.toml wrangler.mail-router.toml wrangler.mail-outbound.toml wrangler.routing-health.toml wrangler.d1-preview.toml scripts/build-router-wasm.ts scripts/build-mail-worker-bundles.ts scripts/preflight.ts scripts/cfctl-v2-command-contract.ts scripts/check-cfctl-provisioning.ts scripts/check-domain-enrollment.ts scripts/compile-fleet-readiness.ts scripts/check-relay-topology.ts scripts/compile-dark-plan.ts scripts/materialize-d1-preview-config.ts scripts/wrangler-config.ts scripts/worker-runtime-provenance.ts scripts/check-maildesk-closeout.ts scripts/collect-live-evidence.ts scripts/plan-mail-proofs.ts scripts/receipt-maildesk.ts scripts/refresh-sender-domain-ack-manifest.ts scripts/apply-sender-domain-ack-manifest.ts scripts/send-mail-probes.ts scripts/verify-maildesk.ts workers/shared/router.ts docs/ARTIFACTS-POLICY.md docs/SCRIPT-OWNERSHIP.md docs/roadmap.md docs/architecture/adr/0001-rust-router-worker-authority.md docs/architecture/template-standard.md docs/architecture/rust-router-contract.md docs/architecture/runtime-contract.md docs/architecture/outbound-identity.md docs/operations/cfctl-contract.md docs/operations/dark-deployment.md docs/operations/deliverability.md docs/operations/horizontal-verifier.md docs/operations/production-rollout.md ops/cfctl/maildesk-cf.surface.md ops/cfctl/maildesk-cf.desired-state.schema.json ops/cfctl/relay-spool-lifecycle.example.json config/policy.example.json config/desired-state.example.json config/domain-enrollment.example.json config/domain-enrollment.schema.json config/fleet-readiness.example.json config/fleet-readiness.schema.json .cfctl/operations/d1-migrations.toml .cfctl/operations/d1-policy-projections.toml; do
  test -s "${ROOT_DIR}/${file}"
  echo "ok ${file}"
done

echo "== domain enrollment"
bun run check:domain-enrollment -- --enrollment config/domain-enrollment.example.json --desired-state config/desired-state.example.json --policy config/policy.example.json --json >/dev/null

echo "== fleet readiness compiler"
bun run compile:fleet-readiness -- \
  --input config/fleet-readiness.example.json \
  --expected-policy-projection-sha256 sha256:ccca416b63ed8c497a7d02b5e59861a7e481b45ed81646168c180944e286be13 \
  --json >/dev/null

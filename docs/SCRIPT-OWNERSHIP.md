...
| `scripts/worker-runtime-provenance.ts` | proof-helper | Reproduces cfctl's Worker artifact-set digest, validates one 100% active deployment plus its exact version-detail annotation, and emits only body-free candidate/runtime hashes. It rejects split traffic, malformed annotations, path escape, symlinks, and unavailable artifacts. |
